...
echo '
<div class="lvepanel '.($disabled?'disabled':$locked).'" id="divcodes">
	...
	<div class="lvepanel-body">
		...
		<input type="text" '.($disabled?'disabled':$locked).' name="codes_ausgegeben" value="'.$db->convert_html_chars(($evaluierung->codes_ausgegeben!=''?$evaluierung->codes_ausgegeben:$anzahl_studierende)).'" size="9">
		<input type="submit" name="saveAusgegeben" '.($disabled?'disabled':$locked).' value="'.$p->t('global/speichern').'" />
		'.$evaluierung_ausgegeben_msg.'
		</form>
	</div>
</div>
...
echo '
	<div class="lvepanel '.($disabled?'disabled':'').'" id="divselbsteval">
		...
		<tr>
			<td colspan="2">
				<textarea name="persoenlich" '.$locked.'>'.$persoenlich.'</textarea>
			</td>
		</tr>
		<tr>
